...
async function main() {
  ...
  if (comments.length === 0) {
    // Check for any stray HTML comments that shouldn't be there
    const { cleaned, stripped } = stripAllHtmlComments(content)
    if (stripped > 0) {
      writeFileSync(draftPath, cleaned, 'utf8')
      console.log(`⚡ No INLINE_IMAGE comments, but stripped ${stripped} stray HTML comment(s). Draft is clean.`)
    } else {
  ...
  // ── FINAL SAFETY SWEEP ────────────────────────────────────────────────────
  // Strip any remaining HTML comments (malformed comments, edge cases, etc.)
